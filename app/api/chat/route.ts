...
function budgetFrom(message: string) {
  const match = message.match(/(?:under|below|less than|max(?:imum)?|budget(?: is| of)?)\s*(?:s(?:gd)?\s*)?\$?\s*([\d,]+(?:\.\d+)?)/i);
...
function relevantProducts(message: string, products: Product[], limit = 6) {
  ...
  const scored = products
    ...
    .map((product) => {
      ...
      const semanticScore = exactPhraseScore + colourScore + terms.reduce((sum, term) => sum + (includesTerm(name, term) ? 4 : includesTerm(attributes, term) ? 3 : includesTerm(category, term) ? 2 : includesTerm(description, term) ? 1 : 0), 0) - unrequestedComplexity;
...
function catalogueRequestLabel(message: string) {
  ...
  const filler = new Set(["any", "are", "available", "below", "budget", "buy", "can", "could", "demo", "do", "does", "exact", "few", "find", "for", "have", "how", "i", "is", "item", "looking", "max", "maximum", "me", "much", "need", "of", "one", "options", "please", "price", "product", "sell", "sgd", "show", "some", "the", "there", "this", "to", "under", "want", "what", "which", "with", "you", "your"]);
