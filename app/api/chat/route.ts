...
async function n8nReply(webhookUrl: string, workflowKey: string | undefined, payload: Record<string, unknown>) {
  ...
  try {
    ...
    return typeof reply === "string" && reply.trim()
      ? reply.trim().slice(0, 3000).replace(/\s*[—–]\s*/g, ", ").replace(/\s{2,}/g, " ")
      : null;
  } catch {
    return null;
  } finally {
